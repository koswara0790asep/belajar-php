<?php
  include "config/db.php";
    ...
    if (!isset($_GET['id'])) {
        echo "<script>alert('ID tidak ditemukan'); window.location.href='user-index.php';</script>";
        exit;
    } else {
        $id = $_GET['id'];
        $user = mysqli_fetch_assoc($conn->query("SELECT users.*, roles.* FROM users JOIN roles ON roles.id = users.role_id WHERE users.id =". $id .";"));
    }
?>
